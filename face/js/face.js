(function($){
    ...
    * */
        ...
        for(var i=0;i<icolLen;i++){
            sHtml += '<div class="swiper-slide">' +
                        '<div class="HyunImg-body">' +
                            '<div class="HyunImg">' +
                                '<video  preload="auto" controls="controls" controls="controls"><source src="'+ row[i].coolFileAddress  +'" /></video>'+
                            '</div>' +
                ...
                '<div class="service-info">' +
                        '<div class="serviceTx">' +
                            '<div class="serviceTx-img">' +
                                '<img src="' + row[i].resourceList[0].resourceUrl + '" />' +
                            '</div>'+
            ...
            sHtml += '<div class="swiper-slide">' +
                        '<div class="articleBdy">' +
                            '<div class="articleLft">' +
                                '<img src="' + row[i].scCmsSubscriptionParam.subscriptionImgUrl + '"/>' +
                                '<div class="articleFc"></div>'+
                            '</div>'+
                            ...
                            for(var a =0;a<row[i].scCmsChannelList.length;a++){
                                sHtml += '<li>' + row[i].scCmsChannelList[a].channelName + '</li>';
                            };
            sHtml += '</ul>' +
                    '<div class="articleInfo">' +
                        '<div class="articleRead"><i class="readIcon"></i><span>'
            if((row[i].totalVisitNumber)>=10000){
    ...
    function getUserInfo(res){
       var row = res.data.userMain;
       var raw = res.data.userMain.scGameUser;
       $("#userpic").attr("src",raw.userpic);//用户头像
       $("#visitCount").html(raw.visitCount);//访问量
       $("#likeCount").html(raw.likeCount);//点赞量
        $("#username").html(raw.username);//用户名
        $("#age").html(row.age + "岁");//用户年龄
        //用户性别
        if(raw.usersex=='女'){
        ...
        $("#signature").html(raw.signature);//用户签名
    ...
    //滚动
    var swiper = new Swiper('.conmonCont', {
